...
volatile uint32_t ip_addr = IP_ADDR;
volatile uint32_t ip_mask = IP_SUBNET_MASK;
volatile uint32_t ip_gateway = IP_DEFAULT_GATEWAY;
...
// UDP listen ports
udp_listenport_t udp_pool[UDP_MAX_LIST_PORTS];
...
{
	uint8_t c;
  for(c = 0; c < UDP_MAX_LIST_PORTS; ++c) {
    if(udp_pool[c].port == htons(lport)) {
       udp_pool[c].port = 0;
      return;
    }
  }
...
void udp_listen(uint16_t lport, udp_callback_t callback)
{
	uint8_t c;
  for(c = 0; c < UDP_MAX_LIST_PORTS; ++c) {
    if(udp_pool[c].port == htons(lport)) return;  
    if(udp_pool[c].port == 0) {
       udp_pool[c].port = htons(lport);
       udp_pool[c].callback = callback;
      return;
    }
  }
}
...
void udp_filter(eth_frame_t *frame, uint16_t len)
{
	...
	uint8_t i;
  udp_listenport_t *plst, *lst=0;

	if(len >= sizeof(udp_packet_t))
	{
		len = ntohs(udp->len) - sizeof(udp_packet_t);
		for(i = 0; i < UDP_MAX_LIST_PORTS; ++i)
    {
      plst = udp_pool + i;
      if(udp->to_port == plst->port)
        {
          lst = plst;
          break;
        }
     }  
    if(lst) lst->callback(frame,len);
	}
}
